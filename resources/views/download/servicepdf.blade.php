...
<div class="row">
                            <div class="col-xs-12">
                                <div class="invoice-title"> 
                                    <div class="pull-right"><h2>Services</h2></div>
                                    <br/>
                                    <br/>
...
@if(count($services)!=0)
<h3>Printed Date: {{date('M j, Y', strtotime($services[0]->current))}}</h3>
<table class="center">
    <thead>
    <tr>
...
<tbody>
@foreach($services as $galleries)
<tr>
    <td>{{$galleries->id}}</td>
    <td>{{$galleries->service_name}}</td>
    <td>{{$galleries->service_description}}</td>
